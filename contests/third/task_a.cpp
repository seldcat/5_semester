...
class Graph {
 public:
  ...
  const std::vector<std::vector<uint32_t>> FloydAlgorithm() const {
    std::vector<std::vector<uint32_t>> shortest_distances = adjacency_matrix_;
...
int main() {
  ...
  std::vector<std::vector<uint32_t>> shortest_distances = graph.FloydAlgorithm();
